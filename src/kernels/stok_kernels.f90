...
subroutine st3d_slp_vec6(src,ndt,targ,ndd,dpars,ndz,zk,ndi, &
     ipars,val)
  implicit real *8 (a-h,o-z)
  implicit integer *8 (i-n)
  real *8 :: src(*), targ(ndt),dpars(ndd)
...
!f2py intent(out) val
  ...
  !
  ! does *not* include the 1/4pi scaling to be consistent
  ! with the FMM. This returns the distinct entries
  ! with the rest given by symmetry. (11,21,31,22,32,33)
  ...
  val(2) = dxdy*over4pi
  val(3) = dxdz*over4pi
  val(4) = (rinv + dy*dy*rinv3)*over4pi
  val(5) = dydz*over4pi
  val(6) = (rinv + dz*dz*rinv3)*over4pi

  return
end subroutine st3d_slp_vec6
...
subroutine st3d_dlp_vec6(srcinfo,ndt,targ,ndd,dpars,ndz,zk,ndi, &
     ipars,val)
!f2py intent(in) src,ndt,targ,ndd,dpars,ndz,zk,ndi,ipars
!f2py intent(out) val
  implicit real *8 (a-h,o-z)
  ...
  !
  ! where r = |src-targ| and n is the normal vector at 
  ! the source. Only the six distinct entries are returned
  ! the others given by symmetry. (11,21,31,22,32,33)
...
subroutine st3d_strac_vec(srcinfo,ndt,targinfo, &
...
subroutine st3d_strac_vec6(srcinfo,ndt,targinfo, &
     ndd,dpars,ndz,zk,ndi,ipars,val)
  implicit real *8 (a-h,o-z)
  implicit integer *8 (i-n)
  real *8 :: srcinfo(*), targinfo(12),dpars(ndd)
...
!f2py intent(out) val
  ...
  !
  ! where r = |src-targ| and n is the normal vector at the
  ! target. The 6 distinct entries, with the rest given
  ! by symmetry. (11,21,31,22,32,33)
...
subroutine st3d_sprime_vec6(srcinfo,ndt,targinfo, &
     ndd,dpars,ndz,zk,ndi,ipars,val)
  implicit real *8 (a-h,o-z)
  real *8 :: srcinfo(*), targinfo(12),dpars(ndd)
  integer *8 ipars(ndi)
...
!f2py intent(out) val
  ...
  !
  ! where r = |src-targ| and n is the normal vector at the
  ! target and r_i = targ_i-src_i. The six distinct entries
  ! with the rest given by symmetry (11,21,31,22,32,33)
...
!  call prin2('srcinfo=*',srcinfo,12)
  ...
  val(2) = dxdy
  val(3) = dxdz
  val(4) = dy*dy*rinv5 + (2*targnorm(2)*dy - dprod)*rinv3
  val(5) = dydz
  val(6) = dz*dz*rinv5 + (2*targnorm(3)*dz - dprod)*rinv3

  return
end subroutine st3d_sprime_vec6
