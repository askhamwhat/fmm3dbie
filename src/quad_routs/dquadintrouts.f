 ...
 1111     continue
     ...
     7          cintvals(1,itarg),ier)
           if(ier.eq.4) then
             ...
             npmax = npmax0
             nqmaxuse = nqmaxuse0
             call prinf('restarting adaptive integration with nquad=*',
     1             nqmaxuse,1)
...
c-------------------
      ...
      real *8, allocatable :: uvtmp(:,:), ctmp(:)
      character *1 transa,transb
      integer lda,ldb,ldc
      external fker
      
      allocate(uvtmp(2,kpols),ctmp(nppols))
...
c        * expected calling sequence,
c            fker(y,ndtarg,x,ndd,dpars,ndz,zpars,ndi,ipars,f)
c        The output 'f' is real *8(nker) for this subroutine
c    - nker: integer
c        size of kernel output
c    - ndd: integer
c        number of real parameters
c    - dpars: real *8 (ndd)
c        real parameters for the fker routine
...
c 
c  Output arguments:
c    - cintvals: real *8 (nker,nppols,ntarg)
c        Integral against all basis functions from patches
c        to targets
c
c------------------------------------- 
c
      ...
      complex *16, intent(in) :: zpars(ndz)
      real *8, intent(in) :: qnodes(2,nqpts),wts(nqpts)
      real *8, intent(out) :: cintvals(nker,nppols,ntarg)
      external fker
...
!
      ...
      integer i,j,ii,ipatch,itarg,lda,ldb,ldc,l,ncols
      integer ntarg0,ntargmax
      real *8 fval
      character *1 transa,transb
...
c
      ...
      integer, intent(in) :: nqorder, nker
      real *8, intent(in) :: rfac
...
c
     ...
     1               ndd,dpars,ndz,zpars,ndi,ipars,fkervals(1,ii))
                do l = 1,nker
                   fkervals(l,ii) = fkervals(l,ii)*qwts(jj)
                enddo
                do j=1,nppols
                  sigmatmp(j,ii) = rsigtmp(j,jj)
                enddo
...
c
      real *8 eps
      integer intype
      integer npatches,norder,npols,nker
      integer nporder,nppols
      real *8 srccoefs(9,npols,npatches)
...
c-------------------
      ...
      implicit real *8 (a-h,o-z)
      integer, allocatable :: istack(:)
      integer ichild_start(nqmax), nker
      real *8 da(nqmax)
      real *8 tvs(2,3,nqmax), uvsq(2,kpols),wts(kpols)
      integer nproclist0, nproclist
      integer idone
...
c
      ...
      allocate(istack(2*nqmax))
      allocate(cvals(nker,ksigpols,nqmax))
...
c
      do i=1,kpols
         call fker(srcvals(1,i),ndtarg,xt,ndd,dpars,ndz,zpars,ndi,
     1        ipars,xkernvals(1,i))
         do l = 1,nker
            xkernvals(l,i) = xkernvals(l,i)*qwts(i)
         enddo
         do j=1,ksigpols
            do l = 1,nker
               cvals(l,j,1) = cvals(l,j,1)
     1              +xkernvals(l,i)*sigvalsdens(j,i)
            enddo
         enddo
      enddo
...
c        number of quads in the initial stack of quads to
c        be processed
c    - xkernvals: real *8(nker,npmax)
c     kernel values on the adaptive integration hierarchy
c     - ctmp: real *8(nker,nppols) work array
c  Output arguments:
c    - cintall: real *8 (nker,nppols)
c        Integral against all basis functions from patches
c        to targets
c    - ier: integer
c        error code
...
c-------------------
      ...
      implicit real *8 (a-h,o-z)
      integer istack(*),nproclist0,nker
      integer ichild_start(nqmax)
      integer nporder,nppols
      real *8 da(nqmax)
      real *8 tvs(2,3,nqmax), uvsq(2,kpols),wts(kpols)
...
c
          ...
          do j=1,kfine
            jj=j+istart
            call fker(srcvals(1,jj),ndtarg,xt,ndd,dpars,
     1           ndz,zpars,ndi,ipars,xkernvals(1,jj))
            do l = 1,nker
...
c          quadrangle
          do isig=1,ksigpols
             do l=1,nker
                cintall(l,isig) = cintall(l,isig)-cvals(l,isig,iquad)
                ctmp(l,isig) = 0
             enddo
          enddo
...
c
          ...
          do isig=1,ksigpols
             do l = 1,nker
                if(abs(ctmp(l,isig)-cvals(l,isig,iquad)).gt.errmax) 
     1               errmax = abs(ctmp(l,isig)-cvals(l,isig,iquad))
             enddo
          enddo
...
c
      ...
      integer nproclist0,nproclist,nquad0,npts0,l
      real *8 zz
      ...
      allocate(cvals(nker,nppols,nquadmax))
      allocate(istack(2*nquadmax))
...
c
     ...
     1               ndd,dpars,ndz,zpars,ndi,ipars,xkernvals(1,jj))
                ...
                do j=1,nppols
                   do l = 1,nker
                      zz = xkernvals(l,jj)*sigvalsdens(j,jj)
                      cvals(l,j,iqquad) = cvals(l,j,iqquad)+zz
                      cintvals(l,j,itarg)=cintvals(l,j,itarg)+zz
                   enddo
                enddo
              enddo
            endif
...
c
     ...
     5         nproclist0,xkernvals,cintvals(1,1,itarg),ier)
