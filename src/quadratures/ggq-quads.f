...
c
     ...
     1   umat,srccoefs,ndtarg,targ,fker,nker,ndd,dpars,ndz,zpars,ndi,
     2   ipars,dquad)
c
c
c------------------
...
c
      ...
      implicit real *8(a-h,o-z)
      implicit integer *8 (i-n)
      integer *8, intent(in) :: norder,npols,ndtarg,nker
      real *8, intent(in) :: srccoefs(9,npols)
      real *8, intent(in) :: targ(ndtarg)
      real *8, intent(in) :: uvs(2),umat(npols,npols)
      integer *8, intent(in) :: ndi,ndd,ndz
      ...
      allocate(fint(nker,npols),sigvalstmp(npols),fvals(nker))
...
c
      do i=1,ns
        ...
        call fker(srctmp(1,i),ndtarg,targ,ndd,dpars,
     2         ndz,zpars,ndi,ipars,fvals)
           
        do j=1,npols
           do l = 1,nker
              fint(l,j) = fint(l,j) + fvals(l)*sigvals(j,i)*qwts(i)
           enddo
        enddo
      enddo
